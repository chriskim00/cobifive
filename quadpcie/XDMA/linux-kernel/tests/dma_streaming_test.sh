...
do
  echo "Info: Writing to h2c channel $i. System Initalization."
  $tool_path/dma_to_device -d /dev/xdma0_h2c_${i} -f data/initialize.bin -s $resetSize -c $resetCount &
done
...
do
  ...
  $tool_path/dma_to_device -d /dev/xdma0_h2c_${i} -f data/cobi_input_data12prob.bin -s $transferSize -c $transferCount &
done
...
for ((i=0; i<$channelPairs; i++))
do
  rm -f data/cobi_response.bin
  echo "Info: DMA setup to read from c2h channel $i. Waiting on write data to channel $i."
  $tool_path/dma_from_device -d /dev/xdma0_c2h_${i} -f data/cobi_response.bin -s $transferSize -c $transferCount &
done
