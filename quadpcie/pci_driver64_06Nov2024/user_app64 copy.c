...
void perform_operations(const char* device_file) {
    ...
    perform_bulk_write(fd, (const uint64_t*)rawData1, RAW_BYTE_CNT);  // Assume rawData is updated to 64-bit
/*
    usleep(1);
    printf("Writing Problem 2.\n");
    perform_bulk_write(fd, (const uint64_t*)rawData2, RAW_BYTE_CNT);  // Assume rawData is updated to 64-bit

    usleep(1);
    printf("Writing Problem 3.\n");
    perform_bulk_write(fd, (const uint64_t*)rawData3, RAW_BYTE_CNT);  // Assume rawData is updated to 64-bit
...
/*
    ...
    perform_bulk_write(fd, (const uint64_t*)rawData4, RAW_BYTE_CNT);  // Assume rawData is updated to 64-bit

    usleep(1);
    printf("Writing Problem 5.\n");
    perform_bulk_write(fd, (const uint64_t*)rawData5, RAW_BYTE_CNT);  // Assume rawData is updated to 64-bit

    usleep(1);
    printf("Writing Problem 6.\n");
    perform_bulk_write(fd, (const uint64_t*)rawData6, RAW_BYTE_CNT);  // Assume rawData is updated to 64-bit

    usleep(1);
    printf("Writing Problem 7.\n");
    perform_bulk_write(fd, (const uint64_t*)rawData7, RAW_BYTE_CNT);  // Assume rawData is updated to 64-bit

    usleep(1);
    printf("Writing Problem 8.\n");
    perform_bulk_write(fd, (const uint64_t*)rawData8, RAW_BYTE_CNT);  // Assume rawData is updated to 64-bit
...
void perform_individual_writes(int fd, const uint64_t* data, size_t count) {
    write_data_t write_data;

    for (size_t i = 0; i < count; ++i) {
        write_data.offset = 9 * sizeof(uint64_t);  // Memory Map Address
        write_data.value = swap_bytes(data[i]);    // 64-bit value
...
void perform_operations(const char* device_file) {
    ...
    printf("Writing Problem 1.\n");
    perform_individual_writes(fd, (const uint64_t*)rawData1, RAW_BYTE_CNT);  // Assume rawData is updated to 64-bit

    usleep(5000); 
    printf("Done writing data to cobi chip. Now reading responses.\n");

    // Read and print response logic
    read_offset = 10 * sizeof(uint32_t);  // read FIFO empty status
