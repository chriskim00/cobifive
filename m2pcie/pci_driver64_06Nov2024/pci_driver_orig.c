...
#define DRIVER_NAME "cobi_chip_driver"
...
static ssize_t pci_mmap_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    uint64_t data;  // Use uint64_t for 64-bit data
    ...
    data = readq(dev->hw_addr + dev->offset);  // Read 64-bit data
