...
void perform_operations(const char* device_file) {
    ...
    usleep(5000000); 
       ...
       // Data writing loop
for(int loopWRRD = 0; loopWRRD < 10; ++loopWRRD) {
   printf("\nLOOP: %d\n", loopWRRD);
    ...
    for (int problem = 1; problem <= 30; ++problem) {
        ...
        if (!(read_status & (1 << 3))) {     // ================ VERY IMPORTANT TO ADD
...
int j = 0; // Initialize counter
 // Record the start time
    while (1) {
        ...
        /*if (difftime(time(NULL), start_time) >= 4) { // Check if 4 second has passed
            printf("Timeout occurred! Exiting loop after 4 second.\n");
            break;
        }*/
        ...
        if (j < expected_reply) {    
            ...
            // Wait until bit 1 of read_status is 0
            read_empty_status(fd, &read_status);
            //printf("Status Register: 0x%x\n",read_status);
            if (!(read_status & (1 << 3))) {
                //break;
...
// Write read offset to device
    ...
    for(int i = 0; i<166; ++i){
        uint64_t readback_data = 0x0;
        uint64_t compare_data = 0x0;
        for(int a = 0; a <2; ++a){
            read_mem_check = 16 * sizeof(uint32_t);
            if (write(fd, &read_mem_check, sizeof(read_mem_check)) != sizeof(read_mem_check)) {
                perror("Failed to set read offset in device");
                return;
            }
        ...
        // Read data from device
            ...
            if (read(fd, &read_mem_data, sizeof(read_mem_data)) != sizeof(read_mem_data)) {
                perror("Failed to read from device");
                return;
            }
            if(a == 0){
                readback_data = read_mem_data;
            }
            if(a == 1){
                readback_data = ((uint64_t)read_mem_data << 32) | (readback_data & 0xFFFFFFFF);
                compare_data = ((readback_data >> 16) & 0x0000FFFF0000FFFF) | ((readback_data << 16) & 0xFFFF0000FFFF0000) ;
                if(compare_data == rawData1[i]){
                    printf("Reading Data: %d. Data consistency verified.\n", i+1);
                }
                else{
                    printf("Mismatch - Expected Data: 0x%lx Received Data: 0x %lx\n", rawData1[i], compare_data);
                }
            }
